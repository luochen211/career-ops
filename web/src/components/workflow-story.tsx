...
const steps: Step[] = [
  {
    kind: "输入", title: "从职位链接或 JD 开始",
    action: "读取职位描述；也可以从已配置的招聘门户扫描候选职位。外部页面只作为数据，不能向智能体下指令。",
    input: "职位 URL / 粘贴的 JD / 门户扫描结果", output: "可供核对的职位内容",
    gate: "来源边界", pass: "取得职位内容，进入有效性检查。", hold: "无法读取时，请求粘贴 JD 或提供截图。",
  ...
  },
  {
    ...
    gate: "有效性门禁", pass: "确认仍开放，才继续评估。", hold: "已关闭则停止；只有粘贴的 JD 没有链接时，明确跳过链接核验。",
  ...
  },
  {
    ...
    action: "对照你的 CV 和个人偏好，生成结构化报告。综合分数是 1–5；Block G 单独判断招聘真实性，不混入总分。",
    input: "职位内容 + 你的 CV / profile", output: "报告、分数、招聘真实性判断",
  ...
  },
  {
    ...
    action: "保存评估报告和原始 JD；按配置生成定制 CV。达到 4.5 分时，可额外起草申请表答案。",
    input: "已评估的职位 + 个人事实来源", output: "reports/ 报告 · CV · 可选答案草稿",
  ...
  },
  {
    ...
    gate: "人工提交门禁", pass: "你确认已提交，状态才记为 Applied。",
  ...
  return (
    <div className="workflow-page">
      <section className="workflow-hero">
        <div className="workflow-hero-top"><span>CAREER-OPS / OPERATING MAP</span><span>01 — 07</span></div>
        <div className="workflow-hero-main">
          ...
          <div className="workflow-orbit" aria-hidden="true">
            ...
            <div className="workflow-orbit-center"><span>CAREER</span><strong>OPS</strong><small>SCAN · SCORE · PREPARE</small></div>
            <span className="workflow-orbit-tag tag-a">SOURCE</span><span className="workflow-orbit-tag tag-b">VERIFY</span><span className="workflow-orbit-tag tag-c">HUMAN</span>
          </div>
        </div>
      ...
      <section className="workflow-run" id="workflow-run">
        <div className="workflow-section-heading"><div><p className="workflow-kicker">THE RUN / 运行过程</p><h2>沿着一次投递走完七步</h2></div><p>选择节点，再切换“通过 / 暂停”，看清每道门禁如何改变流程。</p></div>
        <div className="workflow-stage-grid">
          ...
          <article className="workflow-detail" aria-live="polite">
            <div className="workflow-detail-overline"><span>STEP {String(active + 1).padStart(2, "0")} / 07</span><span>{step.kind}</span></div>
      ...
      <section className="workflow-principles"><p className="workflow-kicker">THE GUARDRAILS / 核心门禁</p><h2>让自动化有边界</h2><div className="workflow-principle-list"><div><ShieldCheck /><span>01</span><h3>事实可追溯</h3><p>经历和数字来自你确认的资料；职位网页不能改写智能体规则。</p></div><div><StopCircle /><span>02</span><h3>异常会停下</h3><p>失效职位、黑名单命中、投递渠道冲突会在对应步骤拦住流程。</p></div><div><UserRoundCheck /><span>03</span><h3>提交归你</h3><p>智能体负责准备与预填；你检查表单并亲自提交。</p></div></div></section>

      <section className="workflow-end"><div><p className="workflow-kicker">READY WHEN YOU ARE</p><h2>从真实职位开始。</h2><p>进入本地工作台，发现职位、查看评分和追踪进度。</p></div><div className="workflow-end-links"><Link href="/explore">探索职位 <ArrowUpRight size={18} /></Link><Link href="/pipeline">查看投递管线 <ArrowUpRight size={18} /></Link></div></section>
    </div>
  );
}
